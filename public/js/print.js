...
function update_total() {
  ...
  $('.price').each(function(i){
    price = $(this).html().replace(" EUR","");
    if (!isNaN(price)) total += Number(price);
  });

  total = roundNumber(total,2);

  $('#subtotal').html(total+" EUR");
...
function update_balance() {
  var due = $("#total").html().replace(" EUR","");
...
function update_price() {
  ...
  var price = row.find('.cost').val().replace(" EUR","") * row.find('.qty').val();
  price = roundNumber(price,2);
  isNaN(price) ? row.find('.price').html("N/A") : row.find('.price').html(price+" EUR");
